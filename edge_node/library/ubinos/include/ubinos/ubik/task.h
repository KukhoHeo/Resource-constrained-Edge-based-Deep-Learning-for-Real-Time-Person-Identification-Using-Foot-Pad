 ...
 *
 * \section tasktest00_sec 간단한 태스크 사용 예제 Simple task example
 *
 ...
 *
 *
 * \section tasktest02_sec 태스크의 휴면sleep 기능 시험 Test on sleep function of task
 *
 ...
 *
 *
 * \section tasktest03_sec 태스크의 중지suspend 및 재시작resume 기능 시험 Test on suspend and resume function of task
 *
 ...
 *
 *
 * \section tasktest04_sec 우선순위 기반 멀티태스킹 기능 시험 Test on priority-based multi-tasking
 *
 ...
 *
 *
 * \section tasktest05_sec 태스크의 우선순위 설정 기능 시험 Test on set priority function of task
 *
 ...
 *
 *
 * \section tasktest06_sec 태스크에 의한 메모리 누수 시험 Test for memory leak by task
 *
 ...
 *
 *
 * \section tasktest07_sec 태스크의 다수 신호 객체 기다리기 기능 시험 Test on wait for multiple signal objects function of task
 *
 ...
 *
 *
 * \section tasktest08_sec 태스크의 남은 제한 시간 가져오기 기능 시험 Test on get remaining timeout tick function of task
 *
 ...
 *
 *
 * \section tasktest09_sec 태스크 중지suspend 및 재시작resume 기능 시험 2 Test 2 on suspend and resume function of task
 *
...
{
	unsigned int type :8; /*!< Object type: OBJTYPE__UBIK_TASK */
 ...
 * 			 -n: n-1 번째 매개변수가 잘못되었음<br>
 */
int task_create_ext(task_pt * task_p, taskfunc_ft func, void * arg,
		int priority, unsigned int stackdepth, const char * name,
		unsigned int tag, unsigned int option);
 ...
 * 						<br>
 *
 * @param	max			동시에 기다릴 수 있는 신호 객체 최대 수 (UBINOS__UBIK__TASK_MAXWAITSIGOBJ_MAX 보다 작거나 같아야 함)<br>
 * 						<br>
 *
 * @return	  0: 성공<br>
 * 			<br>
 ...
 */
int task_waitforsigobjs(void ** sigobj_p, int * sigtype_p, void ** param_p,
		int count, unsigned int waitopt);
 ...
 * 			UBIK_ERR__DEADLOCK		: 데드락 발생 가능성이 있음<br>
 */
int task_waitforsigobjs_timed(void ** sigobj_p, int * sigtype_p,
		void ** param_p, int count, unsigned int waitopt, unsigned int tick);
 ...
 * 			UBIK_ERR__DEADLOCK		: 데드락 발생 가능성이 있음<br>
 */
int task_waitforsigobjs_timedms(void ** sigobj_p, int * sigtype_p,
		void ** param_p, int count, unsigned int waitopt, unsigned int timems);
 ...
 * 			UBIK_ERR__DEADLOCK		: 데드락 발생 가능성이 있음<br>
 */
int task_join_timed(task_pt * task_p, int * result_p, int count,
		unsigned int tick);
