 ...
 *
 * \section msgqtest00_sec 간단한 메시지큐 사용 예제 Simple message queue example
 *
 ...
 *
 *
 * \section msgqtest01_sec 메시지큐 기본 기능 시험 Test on basic functions of message queue
 *
 ...
 *
 * \section msgqtest04_sec 메시지큐에 의한 메모리 누수 시험 Test for memory leak by message queue
 *
...
{
	unsigned int type :8; /*!< Object type: OBJTYPE__UBIK_MSGQ */
 ...
 * 			 -n: n-1 번째 매개변수가 잘못되었음<br>
 */
int msgq_create_ext(msgq_pt * msgq_p, unsigned int msgsize,
		unsigned int maxcount, unsigned int option);
 ...
 * 			UBIK_ERR__TERMINATED	: 기다리던 객체가 제거되었음<br>
 */
int msgq_receive_timedms(msgq_pt msgq, unsigned char * msg,
		unsigned int timems);
