...
#ifndef UBIK_STIMER_H_
#define UBIK_STIMER_H_
 ...
 * @file stimer.h
 *
 * @brief ubik semaphore timer API
 *
 ...
 *
 * \section stimertest00_sec 간단한 세마포어 타이머 사용 예제 Simple semaphore timer example
 *
 ...
 *
 *
 * \section stimertest02_sec 세마포어 타이머의 일회성 알림 기능 시험 Test on periodic notification function of semaphore timer
 *
 ...
 *
 *
 * \section stimertest03_sec 세마포어 타이머에 의한 메모리 누수 시험 Test for memory leak by stimer
 *
...
{
	unsigned int type :8; /*!< Object type: OBJTYPE__UBIK_STIMER */
 ...
 * 			 -n: n-1 번째 매개변수가 잘못되었음<br>
 */
int stimer_set(stimer_pt stimer, unsigned int tick, sem_pt sem,
		unsigned int option);
 ...
 * 			 -n: n-1 번째 매개변수가 잘못되었음<br>
 */
int stimer_setms(stimer_pt stimer, unsigned int timems, sem_pt sem,
		unsigned int option);
 ...
 * 			 -n: n-1 번째 매개변수가 잘못되었음<br>
 */
int stimer_set_signal(stimer_pt stimer, unsigned int tick, signal_pt signal,
		int sigtype, unsigned int option);
 ...
 * 			 -n: n-1 번째 매개변수가 잘못되었음<br>
 */
int stimer_setms_signal(stimer_pt stimer, unsigned int timems, signal_pt signal,
		int sigtype, unsigned int option);
